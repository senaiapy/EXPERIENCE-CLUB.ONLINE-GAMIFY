...
export default function MarcasClient({ initialProducts, total, page, limit }: { initialProducts: BrandProduct[], total: number, page: number, limit: number }) {
  ...
  const BrandProductCard = ({ product }: { product: BrandProduct }) => {
    ...
    return (
      <div className="product-card group relative bg-white dark:bg-gray-800 rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-200 dark:border-gray-700">
        {/* Product ID */}
          ...
          <div className="additional-images flex gap-1 p-2">
            {additionalImages.map((img, index) => (
              <img 
                ...
                className="additional-image w-10 h-10 object-cover rounded cursor-pointer border border-gray-200 dark:border-gray-600 hover:border-emerald-300 dark:hover:border-emerald-400 transition-colors"
        ...
        {/* Product Info */}
        <div className="p-4">
          <h3 className="product-name font-bold text-lg mb-1 truncate text-gray-900 dark:text-white">{product.name}</h3>

          {/* Reference */}
          <div className="product-ref text-xs text-gray-500 dark:text-gray-400 mb-2">
            REF: {product.ref}
          </div>
          ...
          <div className="brand-info flex flex-wrap gap-2 mb-3">
            {product.marca && product.marca !== 'N/A' && (
              <span className="product-marca bg-purple-100 dark:bg-purple-900 text-purple-800 dark:text-purple-200 text-xs font-semibold px-2.5 py-0.5 rounded-full">
                🏷️ {product.marca}
              </span>
            )}
            {product.categoria_marca && product.categoria_marca !== 'N/A' && (
              <span className="product-categoria-marca bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 text-xs font-semibold px-2.5 py-0.5 rounded-full">
          ...
          {/* Description */}
          <p className="product-description text-gray-600 dark:text-gray-300 text-sm mb-3 line-clamp-3">
            {product.descripcion}
          </p>
          ...
          <div className="product-price flex items-center justify-between mb-3">
            <div className="flex flex-col">
              <span className="text-gray-500 dark:text-gray-400 text-xs">Compra</span>
              <span className="buy-price text-green-600 dark:text-green-400 font-bold">
                {formatPrice(product.buy_price)}
              </span>
            </div>
            <div className="flex flex-col">
              <span className="text-gray-500 dark:text-gray-400 text-xs">Venta</span>
              <span className="sell-price text-amber-600 dark:text-amber-400 font-bold">
          ...
          {/* Categories and Tags */}
          <div className="product-details text-xs text-gray-500 dark:text-gray-400 space-y-1">
  ...
  return (
    <div>
      {/* Filters Section */}
      <section className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 mb-8 border border-gray-200 dark:border-gray-700 transition-colors duration-300">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-200 dark:focus:ring-emerald-800 focus:border-emerald-500 dark:focus:border-emerald-400 bg-white dark:bg-gray-700 text-gray-900 dark:text-white transition-colors duration-300"
              value={searchTerm}
      ...
      <section>
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">
            Productos por Marcas <span className="text-gray-500 dark:text-gray-400 text-lg">({total} productos)</span>
          </h2>
        ...
        {filteredProducts.length > 0 ? (
          ...
          <div className="text-center py-12">
            <p className="text-gray-500 dark:text-gray-400 text-lg">No se encontraron productos que coincidan con los filtros.</p>
          </div>
        )}
