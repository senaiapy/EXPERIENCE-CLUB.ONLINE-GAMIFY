...
###############################################################################
# PostgreSQL Database Restore Script for Experience Club
#
...
# Configuration
# Auto-detect which container is running (deploy or dev)
if docker ps --format '{{.Names}}' | grep -q "^clubdeofertas_deploy_postgres$"; then
    CONTAINER_NAME="clubdeofertas_deploy_postgres"
elif docker ps --format '{{.Names}}' | grep -q "^clubdeofertas_dev_postgres$"; then
    CONTAINER_NAME="clubdeofertas_dev_postgres"
elif docker ps --format '{{.Names}}' | grep -q "^clubdeofertas_postgres$"; then
    CONTAINER_NAME="clubdeofertas_postgres"
else
    CONTAINER_NAME="${POSTGRES_CONTAINER_NAME:-clubdeofertas_deploy_postgres}"
fi
...
echo -e "\n${BLUE}═══════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  Experience Club - Database Restore Utility${NC}"
echo -e "${BLUE}═══════════════════════════════════════════════════${NC}\n"
