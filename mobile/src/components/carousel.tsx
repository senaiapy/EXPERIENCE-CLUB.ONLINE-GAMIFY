    ...
    <View className="relative h-[200px] w-full overflow-hidden rounded-lg bg-neutral-100">
      ...
      >
        {carouselSlides.map((slide) => (
          ...
          >
            <Image
              source={{ uri: `${Env.FRONTEND_URL}/${slide.image}` }}
              className="size-full"
              contentFit="cover"
            />
      ...
      <TouchableOpacity
        onPress={prevSlide}
        className="absolute left-2 top-1/2 size-10 -translate-y-1/2 items-center justify-center rounded-full bg-black/30"
        style={{
      ...
      <TouchableOpacity
        onPress={nextSlide}
        className="absolute right-2 top-1/2 size-10 -translate-y-1/2 items-center justify-center rounded-full bg-black/30"
        style={{
          transform: [{ translateY: -20 }],
        }}
      >
