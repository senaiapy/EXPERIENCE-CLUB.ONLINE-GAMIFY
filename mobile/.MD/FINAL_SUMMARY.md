...
- **Fix**: Configure to load from frontend server (port 3060)
- **File**: [src/components/ui/product-card.tsx](src/components/ui/product-card.tsx)

### 4. ✅ Hardcoded URLs Removed
- **Problem**: URLs hardcoded as `http://192.168.0.7:3060`
- **Fix**: Environment variables `API_URL` and `FRONTEND_URL`
- **Files**:
...
| `.env.development` | Added `FRONTEND_URL` |
| `.env.production` | Added `FRONTEND_URL` |
| `env.js` | Added `FRONTEND_URL` to schema |
...
- **Real images** from frontend (port 3060)
...
```bash
adb reverse tcp:3062 tcp:3062 && adb reverse tcp:3060 tcp:3060
