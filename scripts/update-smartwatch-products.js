...
/**
 * Script to update SMARTWATCH products in lista.json
 * - Sets category to "SMARTWATCH" (only if category is empty)
...
const BACKUP_FILE = path.join(__dirname, '../products/lista.backup-smartwatch.json');
const REPORT_FILE = path.join(__dirname, '../products/smartwatch-update-report.json');

console.log('🔍 Starting SMARTWATCH products update...\n');
...
function extractBrandName(productName) {
  ...
  // First word should be "SMARTWATCH"
  if (words.length < 2) {
    return '';
  }
  ...
  // (Currently all are SAMSUNG, but ready for future brands)

  return brandName;
}

// Find and update SMARTWATCH products
let updatedCount = 0;
let skippedCount = 0;
...
data.forEach((product, index) => {
  if (product.name && product.name.trim().startsWith('SMARTWATCH ')) {
...
console.log(`   Total SMARTWATCH products found: ${updatedCount + skippedCount}`);
...
const report = {
  ...
  totalSmartwatchProducts: updatedCount + skippedCount,
...
console.log(`\n   Total SMARTWATCH products: ${updatedCount + skippedCount}`);
