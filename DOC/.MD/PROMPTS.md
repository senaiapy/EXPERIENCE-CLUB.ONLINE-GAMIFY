...
in @admin is a ecommerce panel to show products as Productos, orders as Pedidos , categoriesas Categorias
  ...
  when im running frontend in @frontend in dev mode npm run dev-p, to expose port 3060


  get worksheet m in @products/lista.xls file and convert in jon file  as a model :  {
    "id": "1", =>  increment id init in 1
    "category": "", => "" for all products
    "name": "", => 'name' in lista.xls
    "stockStatus": "En stock",  => for all products
    "referenceId": "189664", => referenceId in lista.xls
    "tags": "", => "" for all products
    "description": "", => "" for all products
    "specifications": "", => "" for all products
    "details": "", => "" for all products
    "price": "130.80", => 'price' in lista.xls
    "price_sale": "143.88", => 'price_sale' in lista.xls
    "images": "", => "" for all products
    "brand_name": ""  => "" for all products
  },  but exclude all products if your 'price_sale' = 0 and place all inresult in a file named lista.json

  get all product in @products/lista.json inited of word PERFUME and place this PERFUME in your category, but only if category is empty, and second word in your brand_name and Create a Node.js script to update these products in the JSON file


  get referenceId in all product in @products/lista.json and search for this referenceId in @products/products.json, and if id exist in products json, get all values of keys as: category, tags, description, specifications, details, images, brand_name , and insert this in @products/lista.json, BUT ONLY IF KEYS IS EMPTY,  and update this list


  get  all products 'name' in @products/lista.json, if your values of one this keys: category, tags, description, specifications, details, images, brand_name; BUT ONLY ARE EMPTY, and search for this name in @products/products.json, and if id exist in products json, get this values and insert this in @products/lista.json, and update this list

  get products_empty_fields.json in @products , with your names key,  search your empty fields in web site, https://www.romapy.com/, search for products detais, with searching with paginations, and insert in this values in empty fields

take the @frontend/db/products.json and change all keys to :
  "id": "", => increment id init in "1" but as string => "1"

"referenceId": => increment id init in "000000001" with 9 digits but as string => "000000001"
"price": 26500, => as strign "2500"
"price_sale":
"images":[
      "/images/versace-eros-flame-masc-edp-200ml.jpg"
    ] => remove [] and /images/ => "versace-eros-flame-masc-edp-200ml.jpg"

    in @frontend the value in guaranies in green text ,display in product cart, is diferent of real value display in product details page, fix this to product card display green text value as the same value of details card green text value

s̶t̶r̶i̶k̶e̶t̶h̶r̶o̶u̶g̶h̶ ̶t̶e̶x̶t̶ 

in @frontend haven a chatboot to send  a  n8n web hook and response, create a n8n json workflow to resolve this function , using a n8n new features to internal tables above to version  1.114.0 and use n8n-mcp